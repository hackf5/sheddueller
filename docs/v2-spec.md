...
This document extends [v1](v1-spec.md). V2 keeps the v1 priority, concurrency, clustering, and cancellation-aware job submission model, then adds resilience and scheduling.
...
- Support recurring schedules that materialize ordinary job instances.
...
- Delayed jobs, retries, and recurring occurrences all produce ordinary job records in the shared store.
...
- `DefaultRetryPolicy` is optional. If it is `null`, jobs and schedules do not retry unless they provide their own retry policy.
...
- `NotBeforeUtc` is optional. If omitted or in the past, the job is immediately eligible for claim.
- `NotBeforeUtc` must be normalized to UTC before persistence.
- Per-job `RetryPolicy` overrides `ShedduellerOptions.DefaultRetryPolicy`.
- The expression must forward its lambda `CancellationToken` parameter to the target service method call.
- The scheduler-provided `CancellationToken` is runtime-owned and is never serialized as part of job payload.
- Callers must not capture an external `CancellationToken` for execution control inside the submitted expression.
- The v1 expression restrictions still apply to delayed and retried jobs.
...
- `CancelAsync` succeeds only when the job is still unclaimed.
- If cancellation wins the race, the job transitions to `Canceled` and is never claimed.
...
- Recurring schedule expressions follow the same cancellation-token requirements as `IJobEnqueuer`.
...
- `CreateOrUpdateAsync` does not cancel already materialized jobs.
- `DeleteAsync`, `PauseAsync`, and `ResumeAsync` are idempotent and return `false` only when the schedule does not exist.
- `DeleteAsync` prevents future occurrences from being materialized, but it does not cancel already materialized jobs.

## Job Model Changes

The v1 job record is extended with at least the following fields:

| Field | Type | Notes |
| --- | --- | --- |
| `NotBeforeUtc` | `DateTimeOffset?` | Earliest claim time for delayed and retried jobs. |
...
| `MaxAttempts` | `int` | Effective total attempt limit for this job. |
...
| `CanceledAtUtc` | `DateTimeOffset?` | Present when a pending job is canceled. |
| `SourceScheduleKey` | `string?` | Set for jobs materialized from recurring schedules. |
| `ScheduledFireAtUtc` | `DateTimeOffset?` | Cron occurrence time that produced the job instance. |

`JobState` in v2 contains:
...
- Delayed jobs and retry-wait jobs remain in `Queued`. Eligibility is controlled by `NotBeforeUtc`.
...
- Terminal transitions, heartbeats, and scheduler-owned interruption releases must validate the current `LeaseToken`. Stale owners must not be allowed to complete, fail, renew, or release a reclaimed job.

## Scheduling And Eligibility

### Immediate And Delayed Jobs

- A job is claimable only when:
  - `State` is `Queued`
  - `NotBeforeUtc` is `null` or less than or equal to the current UTC time
  - all referenced concurrency groups have free capacity
- Delayed jobs are created by setting `NotBeforeUtc` in the future.
- The v1 ordering rule remains in force for claimable jobs: `Priority DESC, EnqueueSequence ASC`.

### Pending Cancellation

- Only `Queued` jobs may be canceled.
- A canceled job never reaches `Claimed`.
- Canceling a recurring schedule occurrence affects only that job instance. It does not modify the recurring schedule definition.

## Retry Semantics

- The effective retry policy for a job is:
  - the per-job or per-schedule policy, when present
...
- Scheduler-owned interruption is not a failed execution and does not consume retry budget when the owner safely releases the job before losing the current lease.
- If `AttemptCount` is still less than `MaxAttempts` after failure processing, the job returns to `Queued` with a new `NotBeforeUtc` based on its retry backoff.
- If `AttemptCount` is equal to `MaxAttempts` after failure processing, the job transitions to `Failed`.
- Retry state is per materialized job instance. Recurring schedules do not share retry budget across occurrences.

## Lease, Heartbeat, And Recovery

### Claiming

- Claiming a job must atomically:
  - transition it from `Queued` to `Claimed`
  - increment `AttemptCount`
  - assign a new `LeaseToken`
  - set `LeaseExpiresAtUtc` to `now + LeaseDuration`
  - reserve all concurrency-group capacity for the job

### Heartbeats

- A node that owns a claimed job must renew its lease before `LeaseExpiresAtUtc`.
...
- That execution token must be linked to:
  - host shutdown
  - loss of local job ownership, including failed lease renewal or confirmed reclaim by another node
- When host shutdown begins, the node must stop claiming new jobs and cancel all in-flight execution tokens.
...
- If user code responds to the scheduler-owned token by throwing `OperationCanceledException` or returning a canceled job, Sheddueller must treat that as scheduler-requested interruption rather than a business exception.
- A scheduler-requested interruption must attempt a lease-token-protected release that returns the job to `Queued`, clears current ownership fields, releases concurrency-group occupancy, removes any retry backoff delay caused by the interrupted attempt, and restores `AttemptCount` to the value before the interrupted claim.
...
- If retries remain, the job transitions back to `Queued`, releases its concurrency-group occupancy, receives a new retry `NotBeforeUtc`, and becomes eligible for reclaim later.
- If no retries remain, the job transitions to terminal `Failed` and releases its concurrency-group occupancy.

### Stale Owners

- A node that loses its lease may continue running the handler briefly. This is allowed by the at-least-once contract.
- If that stale node later attempts to complete, fail, heartbeat, or release the job with an old `LeaseToken`, the store must reject the transition.
- Rejection of stale-owner transitions prevents old owners from corrupting the state of a reclaimed job. It does not prevent duplicate side effects outside Sheddueller.
...
- Each due occurrence materializes exactly one ordinary job record with:
  - inherited priority
...
- Materializing a due occurrence must atomically:
  ...
  - enqueue the occurrence job when allowed
  - advance `NextFireAtUtc` to the next future cron occurrence

### Overlap

- `Allow` means every due occurrence may materialize a new job.
- `Skip` means the current due occurrence is dropped when any non-terminal job from the same `SourceScheduleKey` already exists.
...
- A paused schedule materializes no new jobs.
...
`IJobStore` remains a first-class extension point. V2 requires the abstraction to add support for:

- delayed-job eligibility based on `NotBeforeUtc`
...
1. A job enqueued with future `NotBeforeUtc` remains unclaimable until that time and becomes claimable afterward.
2. A job with no effective retry policy fails terminally after its first failed attempt.
3. A job with a retry policy is requeued with the correct `NotBeforeUtc` after a failed attempt, using fixed or exponential backoff as configured.
4. `AttemptCount` increments on claim, not on terminal transition.
5. Scheduler-owned interruption requeues the job without consuming retry budget when the current lease owner safely releases it.
6. Host shutdown cancellation does not terminally fail a no-retry job when the handler cooperatively exits by cancellation and the release succeeds.
7. A healthy node renews leases successfully and prevents reclaim while heartbeats continue.
8. If a node stops heartbeating, another node eventually recovers the job after lease expiry.
9. Lease expiry consumes the current attempt and either requeues or fails the job based on remaining attempts.
10. A stale owner cannot complete, fail, heartbeat, or release a job after another node has reclaimed it.
11. `CancelAsync` succeeds for a queued job and fails once a claim has already won the race.
...
13. `CreateOrUpdateAsync` preserves pause state and does not cancel already materialized jobs.
14. A recurring schedule with a due fire materializes exactly one ordinary job instance across the cluster.
...
21. Deleting a recurring schedule stops future occurrences but does not cancel already materialized jobs.
...
24. When host shutdown begins, in-flight handlers receive a canceled execution token and the node stops claiming new jobs.
25. When a node loses local ownership of a claimed job, the local execution token is canceled.
