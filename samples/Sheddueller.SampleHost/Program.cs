...
{
    var jobId = await enqueuer.EnqueueAsync<DemoJobService>(
      (service, ct, progress) => service.RunProgressAsync("progress-demo", Job.Context, progress, ct),
      cancellationToken: cancellationToken).ConfigureAwait(false);
...
app.MapPost("/launch/retry-then-succeed", async (IJobEnqueuer enqueuer, CancellationToken cancellationToken) =>
{
    ...
    var jobId = await enqueuer.EnqueueAsync<DemoJobService>(
      (service, ct, progress) => service.RunRetryUntilSuccessAsync(runKey, 2, Job.Context, progress, ct),
...
{
    var jobId = await enqueuer.EnqueueAsync<DemoJobService>(
      (service, ct, progress) => service.RunProgressAsync("many-tags-demo", Job.Context, progress, ct),
      new JobSubmission(
        Priority: 15,
        Tags:
        [
    ...
    CancellationToken cancellationToken) =>
{
    ...
    for (var index = 1; index <= 4; index++)
    {
        ...
        var jobId = await enqueuer.EnqueueAsync<DemoJobService>(
          (service, ct, progress) => service.RunGroupHoldAsync(label, Job.Context, progress, ct),
    ...
    CancellationToken cancellationToken) =>
{
    ...
    if (!await HasNonTerminalJobsInGroupAsync(inspectionReader, GroupKey, cancellationToken).ConfigureAwait(false))
    {
        _ = await enqueuer.EnqueueAsync<DemoJobService>(
          (service, ct, progress) => service.RunIdempotentDemoAsync("idempotent-demo-slot-holder", Job.Context, progress, ct),
          new JobSubmission(
            Priority: 50,
    ...
    var jobId = await enqueuer.EnqueueAsync<DemoJobService>(
      (service, ct, progress) => service.RunIdempotentDemoAsync(WorkLabel, Job.Context, progress, ct),
      new JobSubmission(
        Priority: 25,
        ConcurrencyGroupKeys: [GroupKey],
...
app.MapPost("/launch/recurring", async (IRecurringScheduleManager scheduleManager, CancellationToken cancellationToken) =>
{
    var result = await scheduleManager.CreateOrUpdateAsync<DemoJobService>(
      "demo:recurring",
      "* * * * *",
      (service, ct, progress) => service.RunRecurringAsync(Job.Context, progress, ct),
