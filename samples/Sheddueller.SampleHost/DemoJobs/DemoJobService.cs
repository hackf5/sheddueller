...
public sealed class DemoJobService(DemoJobState state)
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        for (var step = 1; step <= 6; step++)
        {
            ...
            await jobContext.LogAsync(JobLogLevel.Information, $"{label} holding slot {step}/6", cancellationToken: cancellationToken).ConfigureAwait(false);
            progress.Report(percent);
        ...
        CancellationToken cancellationToken)
    {
        ...
        for (var step = 1; step <= 10; step++)
        {
            await Task.Delay(TimeSpan.FromSeconds(1), cancellationToken).ConfigureAwait(false);
            var percent = step * 10;
            await jobContext.LogAsync(JobLogLevel.Information, $"{label} idempotent run {step}/10", cancellationToken: cancellationToken).ConfigureAwait(false);
            progress.Report(percent);
        }
    }
